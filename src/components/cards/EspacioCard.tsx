...
const EstadoBadge = styled.View<{ estado: string }>`
  background-color: ${props => {
    switch (props.estado) {
      case 'libre': return theme.colors.success;
      case 'ocupado': return theme.colors.danger;
      case 'mantenimiento': return theme.colors.warning;
      case 'reservado': return theme.colors.secondary;
...
// COLORES CARD - Ahora usa theme.colors.estado
const getBackgroundColor = (estado: string): string => {
  switch (estado) {
    case 'libre': return theme.colors.estado.libre;
    case 'ocupado': return theme.colors.estado.ocupado;
    case 'mantenimiento': return theme.colors.estado.mantenimiento;
    case 'reservado': return theme.colors.estado.reservado;
...
const EspacioCard: React.FC<EspacioCardProps> = ({ espacio, onPress }) => {
  return (
    ...
    >
      ...
      <EspacioInfo>
        <InfoRow>
          <Ionicons name="location" size={getResponsiveSize(14)} color={theme.colors.gray} />
          <InfoText>{espacio.ubicacion}</InfoText>
        </InfoRow>
        
        <InfoRow>
          <Ionicons name="cash" size={getResponsiveSize(14)} color={theme.colors.primary} />
        ...
        <InfoRow>
          <Ionicons 
            ...
            color={espacio.sensor.estado === 'activo' ? theme.colors.success : theme.colors.danger} 
          />
          <InfoText>
